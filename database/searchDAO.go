...
func GetSearchResult(querie string) (*model.SearchResult, error) {
	if querie == "" {
		return nil, errors.New("emtpy querie")
	}

	//Search for Users (name, description)
	var tmpUsers []model.User
	db_result := global_db.Where("name LIKE ?", "%"+querie+"%").Or("description LIKE ?", "%"+querie+"%").Find(&tmpUsers)
	...
	db_result = global_db.Where("title LIKE ?", "%"+querie+"%").Or("uuid LIKE ?", "%"+querie+"%").Or("description LIKE ?", "%"+querie+"%").Find(&tmpVideos)
	if db_result.Error != nil {
		return nil, db_result.Error
	}
